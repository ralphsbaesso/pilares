	...
	$('.data').datetimepicker({
    	...
    	// cor para as thead
    	  $("thead").css({
        	...
        	// Adiciona uma nova linha para apontar a horas 
           ...
           // Remove linha de apontamento de horas 
...
<div>
	...
	<input type="submit" name="operacao" value="Apontar">
	</form>
</div>
<%
	}
...
<%--Apontar uma ordem de serviço --%>

<%
	if(ctrlDiv.toString().equals("apontar")){
...
<%-- Table atividade --%>
	...
	<h3>
		...
		<input type="checkbox" name="chEncerrar" value="true">
	</h3>
	
	<input type="submit" value="Salvar" name="operacao">&nbsp;<input type="reset" value="Cancelar">
</form>
</div>
<%
		}else{
	...
	<table border="1">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<%=
						sdf.format(os.getDataCadastro().getTime())
					%>
				</td>
				<td>
...
<%
		for(Tarefa tarefa: tarefas){
			for(Apontamento apon: tarefa.getApontamentos()){
%>
			<tr>
				<td>
					<%=apon.getId() %>
...
%>
		...
		<tfoot>
			<tr>
				<td colspan="7">
					Quantidade de Apontamento: <%=tarefas.get(0).getApontamentos().size() %>
				</td>
			</tr>
		</tfoot>
	</table>
...
<%-- voltar ao menu apontemento --%>
<%	
	if(!ctrlDiv.toString().equals("null")){  
%>
<br>
<a href="apontamento.jsp">Voltar ao Menu Apontamento</a>
<%	
	}
%>
