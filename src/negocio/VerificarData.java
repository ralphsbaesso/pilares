...
public class VerificarData implements IStrategy {
	...
	public String processar(Entidade entidade) {
		...
		// verifica data de ordem de serviço
		if(entidade instanceof OrdemDeServico){
			...
			d = d/(24*60*60*1000);
			if(d < 0){
				// data limite é inferior a data atual
				d *= -1;
				return "Data limite é inferiro a data atual em " + d + " Dias!!!";
			}
			...
			if(tarefa.getApontamentos().size() > 0){
				for(int i = 0; i < tarefa.getApontamentos().size(); i++){
					...
					if(d < 0){
						...
						sb.append("Data final é superior a data inicial em " + d + " Horas na linha " + (i + 1));
						sb.append("\n");
					}
				}
			}
